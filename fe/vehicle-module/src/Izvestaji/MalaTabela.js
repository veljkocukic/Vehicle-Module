...
export const MalaTabela = () => {
    ...
    let { dataSmall, formatDate,vremeTab,minsToTime } = useContext(DataContext)
    ...
    return (
        <table className="tg mala-tabela" id="mala-tabela-export">
            ...
            <tbody>
                ...
                {dataSmall.data.map(item => <tr><td>{item.trosak}</td><td>{formatDate(item.datum)}</td>{gorivo && <td>{item.potrosnja}</td>}<td>{vremeTab ? minsToTime(item.vreme) : item.cena}</td>{gorivo && <td>{item.ukupno.toLocaleString() }</td>}</tr>)}
